...
# Plot anomalies on SOH
plt.figure(figsize=(12,6))
plt.plot(data.index, data["bms1_soh"], label="SOH")
plt.scatter(data.index[data["anomaly_flag"]], data["bms1_soh"][data["anomaly_flag"]], 
            color='red', label="Anomaly", marker='x')
plt.xlabel("Time")
plt.ylabel("SOH (%)")
plt.title("SOH with Anomaly Flags")
plt.legend()
plt.tight_layout()
plt.show()

# Optional: print anomaly timestamps
print("Anomaly timestamps:")
print(data.index[data["anomaly_flag"]])